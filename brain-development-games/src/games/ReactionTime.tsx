  ...
  const handleClick = (): void => {
    ...
    if (phase === 'click') {
      ...
      if (newAttempts.length >= target) {
        ...
        // reactionTime is still fully recorded above for analytics either
        // way. In Reflective Mode, we simply don't gate level completion on
        // hitting a speed threshold — slower motor response shouldn't read
        // as failure for elderly/memory-impaired users.
        const meetsThreshold = reflectiveMode ? true : avg <= avgThreshold
        if (!saved.current && meetsThreshold) {
    ...
    <>
      ...
      <div className="bg-gradient-to-br from-red-50 via-orange-50 to-yellow-50 p-8 rounded-2xl shadow-xl">
        <div className="text-center mb-6">
          ...
          {reflectiveMode && (
            <p className="text-sm text-teal-600 mt-1">Reflective Mode is on — every round counts, no speed requirement to pass.</p>
          )}
        </div>
            ...
            {completed ? (
              <>
                ...
                <div className="flex justify-center">
                  <NextLevelButton currentLevel={level} gameId="reaction-time" />
                </div>
              </>
            ) : (
              <>
